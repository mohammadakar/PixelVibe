...
<body class="bg-slate-100 text-slate-900">
    <header class="text-slate-800 shadow-lg">
        <nav>
            <a href="{{route('posts.index')}}" class="nav-link">Home</a>

            @auth
                <div class="relative grid place-items-center" x-data="{open: false}" x-on:click.outside="open=false">
                    <button x-on:click="open=!open" type="button" class="round-btn">
                        <img class="size-9 ml-1" src="https://picsum.photos/200" alt="">
                    </button>
                    <div x-show="open" class="bg-white shadow-lg absolute top-10 right-0 rounded-lg overflow-hidden font-light">
                        <p class="ml-4 mb-2 font-bold text-slate-600">{{auth()->user()->username}}</p>
